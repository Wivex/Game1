...
namespace UnityExtensions
{
    ...
    {
        DateTime m_lastRendered = DateTime.MaxValue;
        ...
        public bool hasElementHeaderFormat => elementHeaderFormat != null;
        ...
        internal ReorderableListDrawer.BackgroundColorDelegate onBackgroundColor;
        ...
        public ReorderableListOfValues(
            ...
            Type elementType)
        : base(
            primaryProperty.serializedObject,
            primaryProperty.Copy(),
            !attribute.disableDragging,
            true,
            !attribute.disableAdding,
            !attribute.disableRemoving)
        {
            this.listType = listType;
            this.elementType = elementType;
            elementHeaderFormat = attribute.elementHeaderFormat;
            showFooterButtons = false;
            // showFooterButtons = (displayAdd || displayRemove) && !attribute.hideFooterButtons;
        ...
        static SerializedProperty[] AcquireSerializedProperties(
            SerializedProperty primaryProperty,
        ...
        static void ResizeArray(SerializedProperty property, int arraySize)
        {
        ...
        void DoCollapsedListBackground(Rect position)
        {
        ...
        void CutElement(int elementIndex)
        {
            if (elementIndex < 0)
                return;
        ...
        bool CanPaste(ClipboardContent clipboardContent)
        {
        ...
        void PasteElement(int elementIndex, ClipboardContent clipboardContent)
        {
            if (elementIndex < 0)
                return;
        ...
        protected GUIContent IconToolbarMinus => defaultBehaviours.iconToolbarMinus;

        protected GUIStyle PreButton => defaultBehaviours.preButton;
        ...
        void DrawHeaderButtons(Rect position)
        {
            position.yMin += 3;
        ...
        void DrawRemoveButton(Rect position)
        {
        ...
        readonly List<float> m_ElementHeights = new List<float>();

        void UpdateElementHeights()
        {
        ...
        void DrawFooterCallback(Rect position)
        {
            if (showFooterButtons)
        ...
        float ElementHeightCallback(int elementIndex)
        {
            return m_ElementHeights[elementIndex];
        }

        protected virtual float drawElementIndent => 0;

        void DrawElementCallback(
            Rect position,
            int elementIndex,
            bool isActive,
            bool isFocused)
        ...
        void DrawElementRows(Rect position, int elementIndex, bool isActive)
        {
            var spacing = EditorGUIUtility.standardVerticalSpacing;
            var loopCounter = 0;
            foreach (var array in serializedProperties)
        ...
        void DrawElementColumns(Rect position, int elementIndex, bool isActive)
        {
        ...
        void DrawElementBackgroundCallback(
            Rect position,
            int elementIndex,
            bool isActive,
            bool isFocused)
        ...
        void DrawEmptyElementCallback(Rect position)
        {
            position.y += 2;
        ...
        void HandleElementEvents(Rect position, int elementIndex)
        {
            var @event = Event.current;
            if (@event == null)
                return;
        ...
        void DoElementContextMenu(Rect position, int elementIndex)
        {
            position.x += 1;
        ...
        List<int> m_NonDragTargetIndices =>
            (List<int>)
            m_NonDragTargetIndicesField
                .GetValue(this);
        ...
        const float borderHeight = 0;

        static float AddElementPadding(float elementHeight)
        {
        ...
        static void RemoveElementPadding(ref Rect position)
        {
            var verticalSpacing = EditorGUIUtility.standardVerticalSpacing;
        ...
        delegate void EndEditingActiveTextFieldDelegate();

        static readonly EndEditingActiveTextFieldDelegate
        EndEditingActiveTextField =
